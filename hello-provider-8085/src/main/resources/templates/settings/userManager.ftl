...
<#--                            <span @click="del(scope.row)" class="cq-tag cq-tag-red">删除</span>-->
    ...
    <script>
        ...
        var delDetail={
            url: "${rc.contextPath}/user/delUserDetail",
            type: "POST",
            dataType: "json",
            contentType: "application/json",
            success: function (result) {
            ...
            methods:{
                ...
                view(row) {
                    ...
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/user/userDetail?type=0&id='+row['id']], //iframe的url，no代表不显示滚动条
                ...
                edit(row) {
                    ...
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/user/userDetail?type=1&id='+row['id']], //iframe的url，no代表不显示滚动条
                ...
                newbulid(){
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/user/userDetail?type=2'], //iframe的url，no代表不显示滚动条
                ...
                resetPass(row){
                    this.$confirm('您确定重置 '+row.name+' 的密码？', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                            resetPassAjax['data']=JSON.stringify(row);
                            $.ajax( resetPassAjax);
                    }).catch(() => {
                        this.$message({
                            type: 'info',
        ...
        function loadTable(){
            loadPage.data.pageIndex=vueobj.currentPage;
            loadPage.data.pageSize=vueobj.pageSize;
            if(vueobj.search!=null&&vueobj.search.trim()!=''){
                loadPage.data.search=vueobj.search;
            }else {
                loadPage.data.search=null;
            }
            $.ajax(loadPage);
        }

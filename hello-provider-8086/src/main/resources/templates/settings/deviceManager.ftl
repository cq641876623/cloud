...
<#--                            <span @click="del(scope.row)" class="cq-tag cq-tag-red">删除</span>-->
    ...
    <script>
        ...
        var delDeviceDetail={
            url: "${rc.contextPath}/device/delDeviceDetail",
            type: "POST",
            dataType: "json",
            ...
            methods:{
                ...
                view(row) {
                    ...
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/device/deviceDetail?type=0&id='+row['id']], //iframe的url，no代表不显示滚动条
                ...
                edit(row) {
                    ...
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/device/deviceDetail?type=1&id='+row['id']], //iframe的url，no代表不显示滚动条
                ...
                newDevice(){
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/device/deviceDetail?type=2'], //iframe的url，no代表不显示滚动条
                ...
                del(row){
                    this.$confirm('此操作将删除设备, 是否继续?', '提示', {
                        ...
                        delDeviceDetail['data']=JSON.stringify(row);
                        $.ajax(delDeviceDetail);
                    }).catch(() => {
                        this.$message({
                            type: 'info',
                            message: '已取消删除'
                ...
                batchDel(){
                    ...
                    this.$confirm('此操作将批量删除设备, 是否继续?', '提示', {
                        ...
                        this.multipleSelection.forEach(function (item) {
                            delDeviceDetail['data']=JSON.stringify(item);
                            $.ajax(delDeviceDetail);
                        });
                    }).catch(() => {
                        this.$message({
        ...
        var loadDevice={
            url: "${rc.contextPath}/device/loadDevicePage",
            type: "GET",
            dataType: "json",
            data:{
        ...
        function loadTable(){
            loadDevice.data.pageIndex=vueobj.currentPage;
            loadDevice.data.pageSize=vueobj.pageSize;
            if(vueobj.search!=null&&vueobj.search.trim()!=''){
                loadDevice.data.search=vueobj.search;
            }else {
                loadDevice.data.search=null;
            }
            $.ajax(loadDevice);
        }

...
<#--                            <span @click="del(scope.row)" class="cq-tag cq-tag-red">删除</span>-->
    ...
    <script>
        ...
        var delDetail={
            url: "${rc.contextPath}/alarmType/delDetail",
            type: "POST",
            dataType: "json",
            ...
            methods:{
                ...
                view(row) {
                    ...
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/alarmType/alarmTypeManagerDetail?type=0&id='+row['id']], //iframe的url，no代表不显示滚动条
                ...
                edit(row) {
                    ...
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/alarmType/alarmTypeManagerDetail?type=1&id='+row['id']], //iframe的url，no代表不显示滚动条
                ...
                newDevice(){
                    layer.open({
                        ...
                        resize:true,
                        offset: 'auto', //右下角弹出
                        content: ['${rc.contextPath}/alarmType/alarmTypeManagerDetail?type=2'], //iframe的url，no代表不显示滚动条
                ...
                del(row){
                    this.$confirm('此操作将删除设备, 是否继续?', '提示', {
                        ...
                        delDetail['data']=JSON.stringify(row);
                        $.ajax(delDetail);
                    }).catch(() => {
                        this.$message({
                            type: 'info',
                ...
                batchDel(){
                    ...
                    this.$confirm('此操作将批量删除设备, 是否继续?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        this.multipleSelection.forEach(function (item) {
                            delDetail['data']=JSON.stringify(item);
                            $.ajax(delDetail);
                        });
                    }).catch(() => {
                        this.$message({
        ...
        var loadAjax={
            url: "${rc.contextPath}/alarmType/loadAlarmTypePage",
            type: "GET",
            dataType: "json",
            data:{
        ...
        function loadTable(){
            loadAjax.data.pageIndex=vueobj.currentPage;
            loadAjax.data.pageSize=vueobj.pageSize;
            if(vueobj.search!=null&&vueobj.search.trim()!=''){
                loadAjax.data.search=vueobj.search;
            }else {
                loadAjax.data.search=null;
            }
            $.ajax(loadAjax);
        }
